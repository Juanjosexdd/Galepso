        ...
        <div class="order-2 lg:col-span-2 lg:order-1">
            ...
            <section class="mb-12">
                ...
                @foreach ($course->sections as $section)
                    {{-- Agrego el condicional en la apertura del article para pedir atravez de loop que en el primer article se mantenga activo --}}
                    ...
                    @else
                        x-data="{open : false}"
                    @endif>
                        <header class="border border-grey-200 px-4 py-2 cursor-pointer bg-grey-200" x-on:click="open = !open">
                            <h1 class="font-bold text-lg text-grey-600">{{$section->name}}</h1>
                        </header>
                        <div class="bg-white py-2 px-4" x-show="open">
                            <ul class="grid grid-cols-1 gap-2">
                                @foreach ($section->lessons as $lesson)
                                    <li class="text-gray-700 text-base"><i class="fas fa-play-circle mr-2 text-gray-500"></i> {{$lesson->name}}</li>
                                @endforeach
                            </ul>
                        </div>
                    </article>
            ...
            <section class="mb-8">
                ...
                <ul class="list-disc list-inside">
                    @foreach ($course->requirements as $requirements)
                        <li class="text-grey-700 text-base"> {{$requirements->name}}</li>
                    @endforeach
                </ul>
            </section>
            ...
            <section class="card mb-4">
                <div class="card-body">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <h1 class="font-bold text-grey-700">Prof. {{$course->teacher->name}}</h1>
                    ...
                    @else
                        <form action="{{route('courses.enrolled', $course)}}" method="POST">
                            @csrf
                            <button class="btn btn-danger btn-block mt-4" type="submit">Ir al curso</button>
                        </form>
                    @endcan
                </div>
            </section>
                ...
                @foreach ($similares as $similar)
                    <article class="flex mb-6">
                        ...
                        <div class="ml-3">
                            ...
                            <p clas="text-sm"><i class="fas fa-star mr-2 text-yellow-400"></i> {{$similar->rating}}</p>
                        </div>
                    </article>
                @endforeach
            </aside>
